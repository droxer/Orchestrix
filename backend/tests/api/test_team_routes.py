...
def _register_node(app, node_id: str, supported: list[str] | None = None) -> None:
    app.state.registry.register(
        {
            ...
            "protocolVersion": 1,
            "supportedAgents": supported or ["codex", "claude"],
            "status": "ready",
        }
    )
...
def test_employee_team_assembly_refuses_members_on_two_computers(monkeypatch) -> None:
    monkeypatch.setenv("RELAY_ADMIN_TOKEN", "admin_token")
    with TemporaryDirectory() as root:
        client = TestClient(create_app(root))
        _bootstrap(client)
        ...
        cross_node = client.post("/api/v1/teams", json=payload)
        assert cross_node.status_code == 400
        assert cross_node.json()["detail"] == "team_members_different_nodes"

        placements.create_placement(support, "node_a")
        assert client.post("/api/v1/teams", json=payload).status_code == 201


def test_a_team_member_cannot_be_moved_or_unplaced_alone(monkeypatch) -> None:
    monkeypatch.setenv("RELAY_ADMIN_TOKEN", "admin_token")
    with TemporaryDirectory() as root:
        app = create_app(root)
        ...
        _bootstrap(client)
        _employee(client, "alice")
        _register_node(app, "node_a")
        _register_node(app, "node_b")
        lead = _agent(client, "alice", "Lead", "codex", place=False)
        support = _agent(client, "alice", "Support", "claude", place=False)
        loner = _agent(client, "alice", "Loner", "codex", place=False)
        placements = app.state.agent_placement_store
        for agent in (lead, support, loner):
        ...
        moved = client.post(
            f"/api/v1/admin/agents/{support['id']}/placements",
            json={"daemonNodeId": "node_b"},
        )
        assert moved.status_code == 409
        assert moved.json()["detail"] == "team_members_different_nodes"

        placement = placements.list_placements(agent_id=support["id"])[0]
        unplaced = client.delete(f"/api/v1/admin/agent-placements/{placement['id']}")
        assert unplaced.status_code == 409
        assert unplaced.json()["detail"] == "team_member_unplaced"
        patched = client.patch(
            f"/api/v1/admin/agent-placements/{placement['id']}",
            json={"desiredState": "removed"},
        )
        assert patched.status_code == 409

        # The team is untouched, and an agent on no team still moves freely.
        assert [
            item["daemonNodeId"]
            for item in placements.list_placements(agent_id=support["id"])
        ] == ["node_a"]
        assert (
            client.post(
                f"/api/v1/admin/agents/{loner['id']}/placements",
                json={"daemonNodeId": "node_b"},
            ).status_code
            == 201
        )
...
def test_a_whole_team_moves_to_another_computer(monkeypatch) -> None:
    monkeypatch.setenv("RELAY_ADMIN_TOKEN", "admin_token")
    with TemporaryDirectory() as root:
        app = create_app(root)
        client = TestClient(app)
        ...
        _register_node(app, "node_a")
        _register_node(app, "node_b")
        _register_node(app, "node_codex_only", supported=["codex"])
        lead = _agent(client, "alice", "Lead", "codex", place=False)
        support = _agent(client, "alice", "Support", "claude", place=False)
        placements = app.state.agent_placement_store
        placements.create_placement(lead, "node_a")
        ...
        assert (
            client.post(
                f"/api/v1/admin/teams/{team['id']}/computer",
                json={"daemonNodeId": "node_missing"},
            ).status_code
            == 404
        )
        blocked = client.post(
            f"/api/v1/admin/teams/{team['id']}/computer",
            json={"daemonNodeId": "node_codex_only"},
        )
        assert blocked.status_code == 409
        assert blocked.json()["detail"] == "team_member_executor_unavailable"

        _login(client, "alice")
        moved = client.post(
            f"/api/v1/teams/{team['id']}/computer", json={"daemonNodeId": "node_b"}
        )
        assert moved.status_code == 200
        for agent in (lead, support):
            assert [
                item["daemonNodeId"]
                for item in placements.list_placements(agent_id=agent["id"])
            ] == ["node_b"]

        _login(client, "bob")
        assert (
            client.post(
                f"/api/v1/teams/{team['id']}/computer", json={"daemonNodeId": "node_a"}
            ).status_code
            == 403
        )
