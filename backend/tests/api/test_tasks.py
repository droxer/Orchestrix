...
def test_task_rejects_invalid_routine_fields(monkeypatch) -> None:
    ...
    with TemporaryDirectory() as root:
        ...
        custom_without_date = client.post(
            "/api/v1/tasks",
            json={
                "title": "Missing custom date",
                "ownerEmployeeId": "alice",
                "assigneeEmployeeId": "alice",
                "assignedAgentId": agent["id"],
                "isRoutine": True,
        ...
        assert custom_without_date.status_code == 400
        assert "next-run date" in custom_without_date.json()["detail"]
