...
def test_registration_persists_machine_id() -> None:
    with TemporaryDirectory() as root:
        registry = _machine_registry(root)
        registry.register(_machine_payload(machineId="machine-a"))
        assert registry.get("node-1")["machineId"] == "machine-a"


def test_registration_without_machine_id_keeps_legacy_workspace_id() -> None:
    with TemporaryDirectory() as root:
        registry = _machine_registry(root)
        registry.register(_machine_payload(workspaceId="machine-a"))
        node = registry.get("node-1")
        assert node["workspaceId"] == "machine-a"
        assert "machineId" not in node


def test_machine_id_survives_a_reregistration_that_omits_it() -> None:
    with TemporaryDirectory() as root:
        registry = _machine_registry(root)
        registry.register(_machine_payload(machineId="machine-a"))
        registry.register(_machine_payload())
        assert registry.get("node-1")["machineId"] == "machine-a"
