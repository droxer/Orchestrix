"""record compatibility release events for repaired deleted agents

Revision ID: 20260729_0045
Revises: 20260729_0044

This forward repair covers databases that ran the initial 0044 data repair
before its event-stream compensation was added. It is a no-op when the latest
event already records the released compatibility identity.
"""
    ...
    rows = (
        connection.execute(
            sa.select(
                agents.c.id,
                agents.c.snapshot,
                agents.c.compatibility_key,
                agents.c.event_version,
            ).where(agents.c.deleted_at.is_not(None))
        )
        .mappings()
        .all()
    )
    for row in rows:
        latest_payload = connection.execute(
            sa.select(events.c.payload).where(
                events.c.agent_id == row["id"],
                events.c.sequence == row["event_version"] - 1,
            )
        ).scalar_one_or_none()
        latest_agent = _snapshot_dict(_snapshot_dict(latest_payload).get("agent"))
        if "compatibilityKey" not in latest_agent:
            continue
...
def downgrade() -> None:
    # Irreversible event repair: removing the compensating event would make the
    # authoritative stream disagree with the already-released identity again.
    pass
