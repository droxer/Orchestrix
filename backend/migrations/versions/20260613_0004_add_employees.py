...
def upgrade() -> None:
    ...
    op.execute("update auth_users set employee_id = username where employee_id is null")
    op.execute(
        """
        insert into employees (id, display_name, email, created_at, updated_at)
        select distinct employee_id, username, email, now(), now()
        from auth_users
