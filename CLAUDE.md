...
- Build: `npm run build` (or `make build`) — builds `relay-core` → `relay-daemon` → `relay-backend` → `relay-tui` → `relay-web`, then compiles `tests/` via the root `tsconfig.json` to `dist/`.
...
- Run the backend: `make backend` (default port `8790`). Serves the control panel at `/cp` and the exported web UI at `/web`.
...
Relay splits into a **backend** (control plane) and **daemons** (execution plane): the backend owns sessions, tasks, and the daemon registry but never executes agents; each daemon registers with the backend, owns its sandbox, and runs the Claude Code, Pi, and Codex CLIs inside it. Everything is TypeScript — **do not add Python host code**.

### Workspace layout (`packages/`)

- **`relay-core`** — Shared protocol and pure helpers. `state.ts` (agent state, `AgentName`), `commands.ts`/`prompts.ts` (agent CLI argv + prompt text), `nodes.ts` (single-agent execution units), `renderers.ts` (streaming JSONL → terminal text), `codex-review.ts` (verdict parsing, `RELAY_REVIEW_VERDICT:` marker), `guest.ts`/`env.ts` (guest auth provisioning, env/`REPO_ROOT`), `daemon-node-protocol.ts` (backend ⇄ daemon command/event types), `daemon-node-token.ts` (per-employee token file under `<workspace>/.relay/daemon-nodes/<employee>.token`), `format.ts` (ANSI helpers).
- **`relay-backend`** — Control plane. Pure backend: it queues work for daemons and never runs agent CLIs or BoxLite itself.
  - `relay/daemon-server.ts` — HTTP backend (`/sandboxes`, `/daemon-nodes`, `/sessions`, `/cp`, `/web`); `relay/daemon-registry.ts` — `DaemonNodeRegistry`; `relay/daemon-backends.ts` — `ServerDaemonNodeBackend` (queues `run.start`/`run.cancel` commands for polling daemons); `relay/daemon-store.ts` — `LocalDaemonStore` (persisted nodes/commands/runs under `.relay/daemon/`); Postgres-backed stores are used automatically when `DATABASE_URL` is set.
  ...
  - `relay/server.ts` — Session/task HTTP API (also mounted by the backend under `/sessions` and `/tasks`).
  - `relay/workflow.ts` — CLI entrypoint (`relay backend|serve|sessions|show|run-workflow`); `relay/routing.ts` — default-workflow handoff transitions (only used by `run-workflow`, which borrows the sandbox session from `relay-daemon`).
...
- **`relay-web`** — Next.js web UI (`basePath: /web`, static export served by the backend at `/web`; dev mode proxies API routes to the backend).
...
- `tests/session.test.ts` — controller + event store behavior, HTTP API, backend + daemon registry (auth, persistence, revival, cancellation).
