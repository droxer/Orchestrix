...
- **`relay-daemon`** — Host daemon and daemon-node runtime.
  - `relay/daemon.ts` — HTTP daemon (`/sandboxes`, `/daemon-nodes`, `/sessions`, `/control`, `/web`), `DaemonNodeRegistry`, `ReverseDaemonNodeBackend` (queues `run.start`/`run.cancel` commands for polling nodes), `LocalDaemonStore` (persisted nodes/commands/runs under `.relay/daemon/`).
