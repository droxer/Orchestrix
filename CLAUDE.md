...
- Run the Python backend: `make backend` (default port `8790`). Serves the control panel at `/cp` and the exported web UI at `/web`. Starts the background task scheduler by default (`RELAY_TASK_SCHEDULER_ENABLED=1`); tune with `RELAY_TASK_SCHEDULER_INTERVAL_SECONDS` and `RELAY_TASK_SCHEDULER_MAX_DISPATCHES`, or disable with `RELAY_TASK_SCHEDULER_ENABLED=0`.
