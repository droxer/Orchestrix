...
- **root `backend/`** — Python control plane. Pure backend: it queues work for daemons and never runs agent CLIs or BoxLite itself.
  - `backend/relay/app.py` — FastAPI backend (`/sandboxes`, `/daemon-nodes`, `/sessions`, `/tasks`, `/cp`, and the web UI at `/`); HTTP routes are split by domain under `backend/relay/api/` — `admin_routes.py` (users, departments, node assignment, agent management, dashboard data), `auth_routes.py`, `chat_routes.py`, `daemon_node_routes.py`, `sandbox_routes.py`, `session_routes.py`, `task_routes.py`, `web_routes.py`.
  - `backend/relay/core/` — shared models, environment loading, ids, logging, and storage config.
  - `backend/relay/persistence/` — event-sourced session, task, and daemon stores (`stores.py`, `session_store.py`, `task_store.py`, `daemon_store.py`, `store_common.py`). Top-level `stores.py`, `session_store.py`, `task_store.py`, and `daemon_store.py` re-export these modules.
...
- **root `web/`** — Next.js web UI (static export served by the backend at the root path `/`; the backend registers the web catch-all last so explicit API routes take precedence; dev mode proxies API routes to the backend). Primary surfaces: chat conversations, task backlog, recurring routines, MCP, skills, and channels. Includes the admin console (`AdminConsole.tsx`) with a dashboard (`components/admin/dashboard/` — `DashboardView`, `KpiTile`, `FleetHealthCard`, `ActivityChart`, `ActivityFeed`, `TokenUsageChart`, `TopEmployees`), node management (`NodeCard`, `AssignNodeDrawer`), and agent management (`ManageAgentsDrawer`).
