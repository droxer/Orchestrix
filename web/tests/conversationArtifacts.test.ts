...
import { visibleConversationArtifacts } from "../src/lib/conversationArtifacts.js";
...
describe("visibleConversationArtifacts", () => {
  it("returns an empty list without a session", () => {
    assert.deepEqual(visibleConversationArtifacts(undefined), []);
  });

  it("excludes command logs", () => {
    const visible = visibleConversationArtifacts(session([
  ...
  it("preserves duplicate generated files as separate conversation artifacts", () => {
    const visible = visibleConversationArtifacts(session([
      artifact({
        id: "art_file_a",
        kind: "workspace_file",
