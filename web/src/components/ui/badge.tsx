...
const badgeVariants = cva(
  /* Badge chrome: caption-bold (12px/700) on a `4px 10px` pad inside the
     ...
     around it — see the radii block in palette.css. The dot-carrying status
     tones below keep their leading dot perfectly round; the dot is the part
     that is supposed to be a circle. */
  "group/badge inline-flex w-fit shrink-0 items-center justify-center gap-1 truncate overflow-hidden rounded-md border px-3 py-0.5 text-micro font-bold whitespace-nowrap transition-[color,box-shadow,border-color,background-color] duration-(--t-fast) ease-(--ease) focus-visible:[outline:var(--focus-outline)] focus-visible:[outline-offset:var(--focus-offset)] focus-visible:ring-0 [&>svg]:pointer-events-none [&>svg]:size-3",
  {
    variants: {
      variant: {
